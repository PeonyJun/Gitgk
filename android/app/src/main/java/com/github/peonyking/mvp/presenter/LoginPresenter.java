...
public class LoginPresenter extends BasePresenter<ILoginContract.View>
    ...
    public void tokenLogin(String token) {
        if (StringUtils.isBlank(token)) {
            mView.onGetTokenError(getContext().getString(
                    com.github.peonyking.R.string.token_cannot_empty));
            return;
        }
        BasicToken basicToken = new BasicToken();
        basicToken.setToken(token.trim());
        basicToken.setScopes(Arrays.asList(AppConfig.OAUTH2_SCOPE.split(",")));
